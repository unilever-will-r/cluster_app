if (FALSE) {
  ...
  attDt <- getAttributionDt(attributionDtPath = config$attributionDtPath, "ICE_CREAM")
  
  storePogDt <- fread("C:/Users/William.Roberts/Unilever/Target POG HUB - Documents/06 - Ice Cream/2024/04.28 April Transvision/03 - Tie Reports/ice_cream_cat_ties.csv")
  indexDt <- storePogDt %>% cleanNames %>%
    .[, .(n_stores = uniqueN(store)), keyby = .(pogId = display, title)]
  
  positionDt <- fread("C:/Users/William.Roberts/Unilever/Target POG HUB - Documents/06 - Ice Cream/2024/04.28 April Transvision/03 - Tie Reports/position_table_nov_9_2023.csv")
  names(positionDt) <- c("pogId", "positionId", "fixtureId", "n_facings", "capacity", "width", "x", "y", "z")
  
  positionDt <- positionDt[order(x,y,z), .(n_facings = sum(n_facings),
                                           capacity = sum(capacity),
                                           width = width[1],
                                           x = x[1],
                                           y = y[1],
                                           z = z[1]),
                           keyby = .(pogId, positionId)]
  
  positionDt <- merge(positionDt, indexDt, all.x = TRUE, by = "pogId") %>%
    merge(attDt[, .(positionId = itemId, brand, item_description)], by = "positionId", all.x = TRUE) %>%
    .[is.na(brand), brand := "UNKNOWN"]
  # Create configs
  configs <- initializeConfigs()
  
  # 1. swaps sheet
  # 1-for-1 swaps
  swapTable <- data.table(currentItems = c(288076242,
                                           288070075,
                                           288070922,
                                           288072228,
                                           288070108,
  ...
  # partial swaps
  # Jenis - put new item in this pog + 100 more stores
  oldItem <- c(288070540, 288070535)
  newItem <- c(288079836, 288070221)
  swapPogs <- positionDt[positionId == oldItem]

  jenisTcaAdds <- c(288079836, 
                    288070221, 
                    288070541,
                    288070169,
                    288070536,
...
if (FALSE) {
  perfLib <- fread("~/Desktop/performance_libraries/icPerfLibrary_latest.csv") %>% cleanNames %>%
    .[, .(pogId = planogram_alias,
          itemId = id,
          nbr_locations,
          usw = unit_movement,
          dsw = value_40)]
